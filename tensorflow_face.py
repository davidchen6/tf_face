...
def dealwithimage(img, h=64, w=64):
    ''' dealwithimage and resize image to required size'''
...
def getface(imgpath, outdir):
    ...
    for f_x, f_y, f_w, f_h in faces:
        ...
        #inx is [[1, 1], [1, 50], [0.5, 0]]index and (alpha, bias) will recall [[1, 1], [1, 50], [0.5, 0]]
...
def main(_):
    ...
    if isneedtrain:
        ...
        pathlabelpair, indextoname = getfileandlabel('./image/testfaces')
    ...
    with tf.Session() as sess:
        ...
        while (n <= 4):
            if (n <= 4):
                ...
                img = cv2.imread(os.path.join('./image/testimages/Ariel_Sharon/', 'Ariel_Sharon_00' + str(n+73) + '.jpg'))                  
